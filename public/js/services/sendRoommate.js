...
.factory("sendRoommate", ['$http',
  ...
  var skip = function(roommate) {
    $http.post('/api/setUserRoommates/skip', {roommate: roommate, type: 'roommatesSeen'})
  ...
  var favorite = function(roommate) {
    $http.post('/api/setUserRoommates/favorite', {roommate: roommate, type: 'roommatesFavorite'})
    .success(function(response, status, headers, config) {
      console.log(response);
    }).error(function(err) { if(err) throw err; });
